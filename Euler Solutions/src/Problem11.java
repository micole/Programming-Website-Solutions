 ...
 * all the nested if's
 * 
 * @author micole What is the greatest product of four adjacent numbers in any
...
public class Problem11 {
	...
	public static void main(String[] args) {
		//Data given to us for this problem
		//It is a 20/20 grid of numbers put into a 2d array
		data = new int[][] {
		...
		for (int row = 0; row < 20; row++) {
			for (int col = 0; col < 20; col++) {
				// Horizontal test
				if (row < 16) {
					sum = data[row][col] * data[row + 1][col]
							* data[row + 2][col] * data[row + 3][col];
					if (sum > largest) {
						largest = sum;
					}

					// forward slash diagonal inside horizontal test
					if (col < 16) {
						sum = data[row][col] * data[row + 1][col + 1]
								* data[row + 2][col + 2]
								* data[row + 3][col + 3];
						if (sum > largest) {
							largest = sum;
				...
				if (row > 4 && col < 16) {
					sum = data[row][col] * data[row - 1][col + 1]
							* data[row - 2][col + 2] * data[row - 3][col + 3];
					if (sum > largest) {
						largest = sum;
					}
				}
				...
				// vertical test
				if (col < 16) {
					sum = data[row][col] * data[row][col + 1]
